...
public:
    ...
    // Added by Banafshe Bamdad. Constructor for RGB-D cameras with my implementation of SuperPoint and LightGlue matcher
    Frame(const cv::Mat &imGray, const cv::Mat &imDepth, const double &timeStamp, SELMSLAM::BBSPExtractor* extractor, ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, GeometricCamera* pCamera, Frame* pPrevF = static_cast<Frame*>(NULL), const IMU::Calib &ImuCalib = IMU::Calib());

    // Constructor for Monocular cameras.
    Frame(const cv::Mat &imGray, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, GeometricCamera* pCamera, cv::Mat &distCoef, const float &bf, const float &thDepth, Frame* pPrevF = static_cast<Frame*>(NULL), const IMU::Calib &ImuCalib = IMU::Calib());

    // Added by Banafshe bamdad. Constructor for Monocular cameras with my implementation of SuperPoint and LightGlue matcher
    Frame(const cv::Mat &imGray, const double &timeStamp, SELMSLAM::BBSPExtractor* extractor,ORBVocabulary* voc, GeometricCamera* pCamera, cv::Mat &distCoef, const float &bf, const float &thDepth, Frame* pPrevF = static_cast<Frame*>(NULL), const IMU::Calib &ImuCalib = IMU::Calib());

    // Destructor
    // ~Frame();

    // Banafshe Bamdad
    void BBExtractSP(int flag, const cv::Mat &im, const int x0, const int x1);
...
public:
    ...
    int Nleft, Nright;
    //Number of Non Lapping Keypoints // B.B keypoints in the frame that are not overlapping with keypoints in adjacent regions or scales???
