...
namespace SELMSLAM {
    ...
    BBLightGlue::BBLightGlue() {
        // cout << endl << "B.B is in BBLightGlue constructor. Press Enter ...";
        // cin.get();
    }
    ...
    void BBLightGlue::CallbackCheckObservations(MapPoint* pMP, Frame& F, const int i) {
        if(F.mvpMapPoints[i]->Observations() <= 0){
    ...
    int BBLightGlue::match(KeyFrame* pKF, Frame &F, vector<MapPoint*> &vpMapPointMatches) {
        ...
        */
        ...
        std::cout << std::endl << "B.B before for loop on matches_info.match1counts..." << std::endl;
        for (int i = 0; i < matches_info.match1counts; i++) {

            std::cout << std::endl << "\tmatch[i]: " << matches_info.match1[i] << ", match score: " << matches_info.mscore1[i] << ", match2[match1[i]]: " << matches_info.match2[matches_info.match1[i]] << ", i: " << i;
    ...
    int BBLightGlue::match(const vector<MapPoint*> &vpMapPoints, cv::Mat MPDescriptors, std::vector<cv::KeyPoint> vMPKeypoint, Frame &F, std::function<void(MapPoint*, Frame&, const int i)> callback) {
        ...
        cout << endl << " ---------- Dimentions of MapPoint/Last Frame ---------- ";
        cout << endl << " ----------  of MapPoints & # of vMPKeypoint & Descriptor dimension rows should be equal ---------- " << endl;
        ...
        cout << endl << " ---------- Dimentions of Current Frame ---------- " << endl;
        cout << "\t# of fKeypoints: " << fKeypoints.size() << endl;
        cout << "\tfDescriptors dimension: (" << fDescriptors.rows << ", " << fDescriptors.cols << ")" << endl;
        ...
        cout << endl << "--- After performig match ---. matches_info.match1counts: " << endl;
