...
#include <iostream> // B.B to write in a text file
...
cv::Mat FrameDrawer::DrawFrame(float imageScale) { // B.B arg is changed by SPT
    cv::Mat im;
    ...
    vector<float> vCurrentDepth; // added by SPT
    float thDepth; // added by SPT
    ...
    cv::Scalar standardColor(0,255,0); // added by SPT
    cv::Scalar odometryColor(255,0,0); // added by SPT

    //Copy variables within scoped mutex
    {
        /**
         * b.b
         * unique_lock object called lock that locks a std::mutex called mMutex.
         * std::mutex: a synchronization primitive used to protect shared resources from concurrent access by multiple threads. 
         * It ensures that only one thread can access the protected code section at a time.
        */
        ...
        */
        if(mState == Tracking::SYSTEM_NOT_READY) {
            ...
            // std::cout << std::endl << "B.B In FrameDrawer::FrameDrawer: IMAGE is ready. mState=" << mState << std::endl;
            // mState = Tracking::OK; // Added by Banafshe
            // B.B mState remains NO_IMAGE_YET. Tracking should change the mState
        }

        mIm.copyTo(im);

        if(mState == Tracking::NOT_INITIALIZED) { // B.B =1

            // std::cout << std::endl << "B.B In FrameDrawer::FrameDrawer: NOT_INITIALIZED" << std::endl;

            vCurrentKeys = mvCurrentKeys;
            vIniKeys = mvIniKeys;
            vMatches = mvIniMatches;
            vTracks = mvTracks;
        } else if(mState == Tracking::OK) { // B.B =2
            ...
            vCurrentDepth = mvCurrentDepth; // added by SPT
            thDepth = mThDepth; // added by SPT

        } else if(mState == Tracking::LOST) { // B.B =4
